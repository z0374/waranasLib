...
function brForP($texto)
{
    $linhas = explode("&#10;", $texto);
    $paragrafos = array_map(
        fn($linha) => '<p class="shl">' . trim($linha) . "</p>",
        $linhas,
    );
    return implode("\n", $paragrafos);
}
...
/**
 * Normaliza uma string mantendo apenas caracteres alfanuméricos.
 *
 * - Remove acentos.
 * - Remove espaços.
 * - Remove símbolos.
 ...
 *
 * Exemplos:
 *   GuestbookPayload   -> GuestbookPayload
 *   Guestbook Payload  -> GuestbookPayload
 *   guestbook_payload  -> guestbookpayload
 *   Guestbook-Payload! -> GuestbookPayload
 *
 * @param string $value
 *
 * @return string
 */
function normAlphaNum(string $value): string
{
    // Translitera para ASCII
    $cleanValue = iconv("UTF-8", "ASCII//TRANSLIT//IGNORE", $value) ?: "";

    //echo "___" . $cleanValue . "---NORMAL VALUE---";

    // Filtra mantendo apenas caracteres alfanuméricos
    return ctype_alnum($cleanValue)
        ? $cleanValue
        : filter_var($cleanValue, FILTER_SANITIZE_URL);
}
