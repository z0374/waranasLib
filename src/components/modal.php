...
function modal($id, $array, $color)
{
    ...
    if (is_array($array)) {
        $imdl .= "<a class='mbt' href='/' id='{$distinctId}{$bt}'>{$array[$length -
                1]["content"]}</a>";
    } else {
    ...
    $body[] =
        "<input type='checkbox' id='modal-toggle{$distinctId}' class='modal-toggle' /><div id='{$distinctId}Mdl' class='mdl {$lowerId}'>" .
        implode("", $mdl) .
        "</div>";
